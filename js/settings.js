...
(function () {
  ...
  function fire(key, val) {
    ...
    if (String(key).indexOf("chm-pet") === 0) window.dispatchEvent(new Event("tw:pet"));
  }
  ...
  panel.innerHTML =
    ...
    settingRow("THEME", '<span class="seg" data-pref="chm-theme">' +
      '<button data-val="dark">DARK</button><button data-val="light">LIGHT</button>' +
      '<button data-val="system">SYSTEM</button></span>') +
    settingRow("ACCENT", '<span class="seg accent-seg" data-pref="chm-accent">' +
    ...
    settingRow("MODE", '<span class="seg" data-petpref="chm-pet">' +
    ...
    settingRow("NAP", petToggle("chm-pet-nap")) +
    settingRow("FLEE", petToggle("chm-pet-flee")) +
    settingRow("READ", petToggle("chm-pet-read")) +
    settingRow("TRICKS", petToggle("chm-pet-tricks")) +
    settingRow("SPEECH", petToggle("chm-pet-speech")) +
    "</div></div>";
  ...
  function petDefaults(key) {
    return { "chm-pet": "float", "chm-pet-nap": "on", "chm-pet-flee": "on",
      "chm-pet-read": "on", "chm-pet-tricks": "on", "chm-pet-speech": "off" }[key];
  }
  ...
  function refreshSeg() {
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      var cur;
      if (key === "chm-theme") cur = TW.prefs.get("chm-theme", "system", ["dark", "light", "system"]);
      else cur = TW.prefs.get("chm-accent", "0", ["1", "2", "3", "4", "5"]);
  ...
  function refreshPet() {
    ...
    document.getElementById("pet-size").value = TW.prefs.get("chm-pet-size", "28");
    document.getElementById("pet-opacity").value = TW.prefs.get("chm-pet-opacity", "70");
  }
  ...
  panel.addEventListener("click", function (e) {
    ...
    if (key === "chm-theme") {
      if (val === "system") { TW.prefs.remove("chm-theme"); }
      else { TW.prefs.set("chm-theme", val); }
      applyTheme(val === "system" ? null : val);
      refreshSeg();
      return;
    }
    if (key === "chm-accent") {
      if (val === "0") { TW.prefs.remove("chm-accent"); }
      else { TW.prefs.set("chm-accent", val); }
